...
<html>
  <head>
    ...
    <style>
      ...
      .collapse-btn:hover { background: #2D3344; }
      .collapse-btn svg { transition: transform 0.2s; }
      .panel.collapsed .collapse-btn svg { transform: rotate(-180deg); }

      /* When collapsed, hide everything below the header */
      .panel.collapsed .action,
      .panel.collapsed .clear-link-row,
      .panel.collapsed .list,
      .panel.collapsed .error { display: none; }
      ...
      /* Clear all link (appears below action area when bookmarks exist) */
      .clear-link-row {
        padding: 0 16px 12px;
        text-align: right;
        border-bottom: 1px solid #3A3F4E;
      ...
      /* Confirmation overlay (covers the panel body) */
      ...
      .btn-confirm-cancel:hover { background: #353C50; }
      .btn-confirm-delete {
        flex: 1;
        height: 36px;
        background: #B91C1C;
  ...
  <body>
    <div class="panel" id="panel">
      ...
      <div class="header">
        ...
        <button id="collapseBtn" class="collapse-btn" aria-label="Toggle">
      ...
      <!-- Clear all link (hidden when list is empty) -->
      <div id="clearLinkRow" class="clear-link-row hidden">
        <button id="clearAllBtn" class="btn-link">Clear all bookmarks</button>
      </div>
      ...
      <div id="confirmOverlay" class="confirm-overlay hidden">
        <div class="confirm-title">Delete all bookmarks?</div>
        <div class="confirm-body">This will permanently remove every saved bookmark. This action cannot be undone.</div>
        <div class="confirm-buttons">
          <button id="confirmCancelBtn" class="btn-confirm-cancel">Cancel</button>
          <button id="confirmDeleteBtn" class="btn-confirm-delete">Delete all</button>
        </div>
      </div>
    </div>
    ...
    <script>
      ...
      const clearLinkRow = document.getElementById("clearLinkRow");
      ...
      function setConfirmMode(on) {
        confirmOverlay.classList.toggle("hidden", !on);
      }
      clearAllBtn.addEventListener("click", () => setConfirmMode(true));
      confirmCancelBtn.addEventListener("click", () => setConfirmMode(false));
      confirmDeleteBtn.addEventListener("click", () => {
        parent.postMessage({ type: "clearAll" }, "*");
      ...
      window.addEventListener("message", (e) => {
        ...
        if (msg.type === "state") {
          ...
          clearLinkRow.classList.toggle("hidden", bookmarks.length === 0);
        }
      });
...
const DEFAULT_FLY_DURATION = 2;

// Read fly-to duration from the inspector. Falls back to default if unset or invalid.
const getFlyToDuration = () => {
  const v = reearth.extension.widget?.property?.default?.flyToDuration;
...
const isValidBookmark = (b) =>
  ...
  typeof b.camera.heading === "number" &&
  typeof b.camera.pitch === "number" &&
  typeof b.camera.roll === "number";
...
const handleSave = async ({ name }) => {
  ...
  if (!camera) {
    storageError = "Camera position is not available right now.";
    sendState();
    return;
  }
...
const handleFlyTo = ({ id }) => {
  ...
  if (!b) return;
  reearth.camera.flyTo(b.camera, { duration: getFlyToDuration() });
};
